...
int main(int argc, char *argv[]){
  ...
  if(argc != 9){ cerr << "usage: <filename> <t0> <tmin> <tmax> <noise_cutoff> <fitCrit> <min_tslices> <svdcut>" << endl; exit(1); }
  ...
  double svdcut; {istringstream val(argv[8]); val >> svdcut;}
  ...
  if( fitCorr.getNExp() == 2 ){
     ...
     cout << "   A = " << toDouble(mean(fitCorr.getA())) << " +/- " << toDouble(sqrt(variance(fitCorr.getA()))) << endl;
  }
  cout << "**********************************************************" << endl;

   //write the log to a file
  {
    stringstream s; s << filen << "_prin_corr_exp_fit.log"; 
  ...
  {  
    stringstream ss; ss << filen << "_prin_corr_exp_fit.ax"; 
  ...
  {  ostringstream filename;  filename << filen << "_prin_corr_exp_fit_mass0.jack";  write(filename.str(), out );  } 
  
  if(fitCorr.getNExp() == 2){
    out = fitCorr.getMass1();
    {  ostringstream filename;  filename << filen << "_prin_corr_exp_fit_mass1.jack";  write(filename.str(), out );  }
    out = fitCorr.getA();
    {  ostringstream filename;  filename << filen << "_prin_corr_exp_fit_A.jack";  write(filename.str(), out );  }
  }
