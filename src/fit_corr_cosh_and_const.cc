...
int main(int argc, char *argv[]){
  ...
  if(argc != 9){ cerr << "usage: <filename> <T> <tmin> <tmax> <noise_cutoff> <fitCrit> <min_tslices> <covar_SV_cutoff>" << endl; exit(1); }
  ...
  double SVcutoff;  {istringstream val(argv[8]); val >> SVcutoff;}
  ...
  cout << "   c = " << setprecision(4) << toDouble(mean(fitCorr.getConst())) << " +/- " << toDouble(sqrt(variance(fitCorr.getConst()))) << endl;
  ...
  {
    stringstream s; s << filen << "_cosh_and_const_fit.log"; 
  ...
  {  
    stringstream ss; ss << filen << "_cosh_and_const_fit.ax"; 
  ...
  {  ostringstream filename;  filename << filen << "_cosh_and_const_fit_mass0.jack";  write(filename.str(), out );  } //cout << "wrote " << filename.str() << endl; }
  out = fitCorr.getAmp0();
  {  ostringstream filename;  filename << filen << "_cosh_and_const_fit_amp0.jack";  write(filename.str(), out );   }//cout << "wrote " << filename.str() << endl; }
  out = fitCorr.getConst();
  {  ostringstream filename;  filename << filen << "_cosh_and_const_fit_const.jack";  write(filename.str(), out );   }//cout << "wrote " << filename.str() << endl; }

  if(fitCorr.getNExp() == 2){
    out = fitCorr.getMass1();
    {  ostringstream filename;  filename << filen << "_cosh_and_const_fit_mass1.jack";  write(filename.str(), out );  }//cout << "wrote " << filename.str() << endl; }
    out = fitCorr.getAmp1();
    {  ostringstream filename;  filename << filen << "_cosh_and_const_fit_amp1.jack";  write(filename.str(), out );  }//cout << "wrote " << filename.str() << endl; }
  }
