...
int main(int argc, char *argv[]){
  ...
  //do the fit
  FitCorrelatorExpAndConst fitCorr(corrData, dum[fitCrit], cutoff, minTSlices); 
  ...
  {
    stringstream s; s << filen << "_exp_and_const_fit.log"; 
  ...
  {  
    stringstream ss; ss << filen << "_exp_and_const_fit.ax"; 
  ...
  {  ostringstream filename;  filename << filen << "_exp_and_const_fit_mass0.jack";  write(filename.str(), out );  } //cout << "wrote " << filename.str() << endl; }
  out = fitCorr.getAmp0();
  {  ostringstream filename;  filename << filen << "_exp_and_const_fit_amp0.jack";  write(filename.str(), out );   }//cout << "wrote " << filename.str() << endl; }
  out = fitCorr.getConst();
  {  ostringstream filename;  filename << filen << "_exp_and_const_fit_const.jack";  write(filename.str(), out );   }//cout << "wrote " << filename.str() << endl; }

  if(fitCorr.getNExp() == 2){
    out = fitCorr.getMass1();
    {  ostringstream filename;  filename << filen << "_exp_and_const_fit_mass1.jack";  write(filename.str(), out );  }//cout << "wrote " << filename.str() << endl; }
    out = fitCorr.getAmp1();
    {  ostringstream filename;  filename << filen << "_exp_and_const_fit_amp1.jack";  write(filename.str(), out );  }//cout << "wrote " << filename.str() << endl; }
  }
