...
namespace SEMBLE
{
  ...
  // Create db keys from operator list
  template<typename K>
  std::vector<K> createRedstarKeys(const Array<string>& opsxmlfiles, InputPropsRedstarKeys_t keyParams, const Array<int> avgRows, bool avgMom);


  // Load from database
  template<typename K>
  vector<SembleMatrix<double> > 
  loadFromRedstarDB(const string &dbfile, vector<string> opsList, const Array<string>& opsxmlfiles,
		    Array<int> avgRows, double avgTol, const string &badlistfile, bool avgMom,
		    InputPropsRedstarKeys_t keyParams)
  {
    ...
    std::string dbtype = getDBType(dbfile);

    try
    {
      // Open DB
      FILEDB::AllConfStoreDB< SerialDBKey<K>,  SerialDBData<EnsemScalar<EnsemVectorComplex>::Type_t> > database;
	...
	exit(1);
      ...
      // Construct the keys to extract by iterating over operators
      std::vector<K> keys = createRedstarKeys<K>(opsList, opsxmlfiles, keyParams, avgRows, avgMom);
      cout << __func__ << ": Will extract nkeys = " << keys.size() << endl;
      ...
      // Find number of time slices and check size of ensemble
      EnsemVectorComplex Test = printKeyValue<K, EnsemVectorComplex>(keys[0], database);
      int Lt = Test.numElem();
      ...
      return loadCorrs(database, keys, avgRows, avgTol, badlistfile, avgMom);
    }
    catch(const std::string &e)
    {
      std::cerr << __func__ << ": Caught Exception: " << e << std::endl;
  ...
  void SembleCCorrs::loadFromDB(const string &dbfile, vector<string> opsList_, vector<int> opsListCConj_, const Array<string>& opsxmlfiles, Array<int> avgRows, double avgTol, const string &badlistfile, bool avgMom, vector< Array<int> > momList_, InputPropsRedstarKeys_t keyParams)
  {
    ...
    // NEED TO check on use of   Lt and nbins
    ...
    try
    {
      ...
      {
	Ct = loadFromRedstarDB<Hadron::KeyHadronNPartNPtCorr_t>(dbfile, opsList, opsxmlfiles, avgRows, avgTol, badlistfile, avgMom, keyParams);
      }
      else if (dbtype == "hadronSUNNPartNPtCorr")
      {
	Ct = loadFromRedstarDB<Hadron::KeyHadronSUNNPartNPtCorr_t>(dbfile, opsList, opsxmlfiles, avgRows, avgTol, badlistfile, avgMom, keyParams);
      }
      else
      {
	std::cerr << __func__ << ": unsupported type of correlator db = " << dbtype << std::endl;
	exit(1);
      }
  ...
  template<typename K>
  std::vector<K> readOpsXml(const Array<string>& opsxmlfiles, const std::vector<std::string>& opsList)
  {
    ...
    // Ops in a map
    std::map<std::string, K> opsxml;

    for(auto xml = opsxmlfiles.ref().begin(); xml != opsxmlfiles.ref().end(); ++xml)
    {
      std::map<std::string, K> tmp;
      
      try
      {
	XMLReader xml_in(*xml);
	read(xml_in, "/OpsList", tmp);
      }
      catch(const std::string &e)
      {
	...
	exit(1);
      ...
      cout << __func__ << ": read " << tmp.size() << " elems from " << *xml << endl;

      opsxml.insert(tmp.begin(); tmp.end());
    }
  ...
  // Create db keys from operator list
  template<>
  std::vector<Hadron::KeyHadronNPartNPtCorr_t> createRedstarKeys<Hadron::KeyHadronNPartNPtCorr_t>(const Array<string>& opsxmlfiles, InputPropsRedstarKeys_t keyParams, const Array<int> avgRows, bool avgMom)
  {
    int nkeys = dim * dim * irrepdim;

    if(avgMom) nkeys *= momList.size();

    std::vector<Hadron::KeyHadronNPartIrrep_t> opsxml = readOpsXml<Hadron::KeyHadronNPartNPtCorr_t>(opsxmlfiles, OpsList);
	...
	for(int i = 0; i < irrepdim; i++)
	{
	  for(int i_mom = 0; i_mom < momList.size(); i_mom++)
	  {
	    ...
	    key.ensemble = keyParams.ensemble;
	    key.npoint.resize(2);  // This is a 2-point correlator. N.B. THIS IS Array1d0 - A ONE BASED ARRAY

	    // The sink op
	    key.npoint[1].t_slice           = -2;
	    key.npoint[1].irrep.row         = avgRows[i];
	    key.npoint[1].irrep.twoI_z      = keyParams.twoI_z;
	    key.npoint[1].irrep.mom         = momList[i_mom];
	    key.npoint[1].irrep.creation_op = false;
	    key.npoint[1].irrep.smearedP    = opsxml[j_snk].smearedP;
	    key.npoint[1].irrep.op          = opsxml[j_snk].op;

	    // The source op
	    key.npoint[2].t_slice           = keyParams.source_tslice;
	    key.npoint[2].irrep.row         = avgRows[i];
	    key.npoint[2].irrep.twoI_z      = keyParams.twoI_z;
	    key.npoint[2].irrep.mom         = momList[i_mom];
	    key.npoint[2].irrep.creation_op = true;
	    key.npoint[2].irrep.smearedP    = opsxml[j_src].smearedP;
  ...
  // Create db keys from operator list
  template<>
  Array<Hadron::KeyHadronSUNNPartNPtCorr_t> createRedstarKeys<Hadron::KeyHadronSUNNPartNPtCorr_t>(const Array<string>& opsxmlfiles, InputPropsRedstarKeys_t keyParams, const Array<int> avgRows, bool avgMom)
  {
    int nkeys = dim * dim * irrepdim;

    if(avgMom) nkeys *= momList.size();

    std::vector<Hadron::KeyHadronSUNNPartIrrep_t> opsxml = readOpsXml<Hadron::KeyHadronSUNNPartNPtCorr_t>(opsxmlfiles);
	...
	for(int i = 0; i < irrepdim; i++)
	{
	  for(int i_mom = 0; i_mom < momList.size(); i_mom++)
	  {
	    ...
	    // The sink op
	    key.npoint[1].t_slice           = -2;
	    key.npoint[1].irrep.irmom.row   = avgRows[i];
	    key.npoint[1].irrep.irmom.mom   = momList[i_mom];
	    key.npoint[1].irrep.flav.two    = keyParams.twoI;
	    key.npoint[1].irrep.flav.threeY = keyParams.threeY;
	    key.npoint[1].irrep.flav.twoI_z = keyParams.twoI_z;
	    key.npoint[1].irrep.creation_op = false;
	    //key.npoint[1].irrep.smearedP    = opsxml[j_snk].smearedP;
	    ...
	    // The source op
	    key.npoint[2].t_slice           = keyParams.source_tslice;
	    key.npoint[2].irrep.irmom.row   = avgRows[i];
	    key.npoint[2].irrep.irmom.mom   = momList[i_mom];
	    key.npoint[2].irrep.twoI        = keyParams.twoI;
	    key.npoint[2].irrep.threeY      = keyParams.threeY;
	    key.npoint[2].irrep.twoI_z      = keyParams.twoI_z;
	    key.npoint[2].irrep.creation_op = true;
  ...
  void read(XMLReader &xml, const std::string &path, std::map<std::vector, Hadron::KeyHadronNPartIrrep_t>& param)
  {
    // Read an array of these objects
    std::vector<opsxml_t> tmp;
    read(xml, path, param);
    ...
    {
      param.insert(std::make_pair(tt->opnamekey, tt->irrep));
    }
  }
